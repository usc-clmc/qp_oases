...
void mexFunction( int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[] )
{
	...
	if ( ( strcmp( typeString,"i" ) == 0 ) || ( strcmp( typeString,"I" ) == 0 ) )
	{
		...
		if ( mxIsSparse( prhs[1] ) != 0 )
		{
			...
			/* copy values, too */
			globalQP_Hir = new sparse_int_t[oct_jc[nV] + nV];
			globalQP_Hjc = new sparse_int_t[nV+1];
			globalQP_Hv = new real_t[oct_jc[nV] + nV];
			for (j = 0; j < nV; j++) 
			{
				globalQP_Hjc[j] = oct_jc[j] + nfill;
				/* fill up to diagonal */
				for (i = oct_jc[j]; i < oct_jc[j+1] && oct_ir[i] <= j; i++) 
				{
					globalQP_Hir[i + nfill] = oct_ir[i];
			...
			globalQP_Hjc[nV] = oct_jc[nV] + nfill;
		...
		if ( nC > 0 )
		{
			...
			if ( mxIsSparse( prhs[3] ) != 0 )
			{
				...
				/* copy indices to avoid 64/32-bit integer confusion */
				globalQP_Air = new sparse_int_t[oct_jc[nV]];
				globalQP_Ajc = new sparse_int_t[nV+1];
				for (long i = 0; i < oct_jc[nV]; i++) globalQP_Air[i] = oct_ir[i];
				for (long i = 0; i < nV + 1; i++) globalQP_Ajc[i] = oct_jc[i];
